...
function AcceptAccesorUserInviteSignup() {
  ...
  const {data:retrievedUserInvite,isError,error} = useGetAccesorUserInviteDetailsQuery(params.get("token"));
  const [backendValErrors,setBackendValErrors]=useState({});
  const [registerAccesorUser,{isLoading:loadingValuer}] = useRegisterAccesorUserMutation();
...
const submitRegister = async (data)=>{
    ...
    formdata.append("organization",retrievedUserInvite.report_consumer_id);
    formdata.append("full_name",data.full_name);
    formdata.append("invite_token",retrievedUserInvite.invite_token);
    const result = await registerAccesorUser(formdata);
    console.log(result);
    if ('error' in result) {
  ...
  return ( 
    <Box display={'flex'} flexDirection={'column'}
        ...
        <form onSubmit={handleSubmitRegisterValuer(submitRegister)} >
          ...
          <Typography variant='p' sx={{ mb: 4 }} align='center'>Creating An Account</Typography>
          ...
          type='submit'
            ...
            size='large' >Complete registration</Button>
...
export default AcceptAccesorUserInviteSignup;