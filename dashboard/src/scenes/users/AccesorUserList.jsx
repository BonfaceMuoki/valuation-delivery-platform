...
import { useSendAccesorFirmUserInviteMutation } from 'features/sendAccesorFirmUserInviteSlice';
...
function AccesorUserList() {
  ...
  const [inviteAccesorUser, { isLoading: sendingUploaderInvite }] = useSendAccesorFirmUserInviteMutation();
  const onSubmitInviteFormsubmit = async (data) => {
    ...
    formdata.append("login_url", `${process.env.REACT_APP_FRONT_BASE_URL}/complete-accesor-user-registration-login`);
    formdata.append("registration_url", `${process.env.REACT_APP_FRONT_BASE_URL}/complete-accesor-user-registration-register`);
    const result = await inviteAccesorUser(formdata);
    if ('error' in result) {
      toastMessage(result.error.data.message, "error");
      if ('backendvalerrors' in result.error.data) {
        // setBackendValErrors(result.error.data.backendvalerrors);
...
export default AccesorUserList